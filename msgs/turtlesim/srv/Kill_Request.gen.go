// Code generated by rclgo-gen. DO NOT EDIT.

package turtlesim_srv
import (
	"unsafe"

	"github.com/tiiuae/rclgo/pkg/rclgo"
	"github.com/tiiuae/rclgo/pkg/rclgo/types"
	"github.com/tiiuae/rclgo/pkg/rclgo/typemap"
	primitives "github.com/tiiuae/rclgo/pkg/rclgo/primitives"
	
)
/*
#include <rosidl_runtime_c/message_type_support_struct.h>

#include <turtlesim/srv/kill.h>

*/
import "C"

func init() {
	typemap.RegisterMessage("turtlesim/Kill_Request", Kill_RequestTypeSupport)
	typemap.RegisterMessage("turtlesim/srv/Kill_Request", Kill_RequestTypeSupport)
}

type Kill_Request struct {
	Name string `yaml:"name"`
}

// NewKill_Request creates a new Kill_Request with default values.
func NewKill_Request() *Kill_Request {
	self := Kill_Request{}
	self.SetDefaults()
	return &self
}

func (t *Kill_Request) Clone() *Kill_Request {
	c := &Kill_Request{}
	c.Name = t.Name
	return c
}

func (t *Kill_Request) CloneMsg() types.Message {
	return t.Clone()
}

func (t *Kill_Request) SetDefaults() {
	t.Name = ""
}

func (t *Kill_Request) GetTypeSupport() types.MessageTypeSupport {
	return Kill_RequestTypeSupport
}

// Kill_RequestPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type Kill_RequestPublisher struct {
	*rclgo.Publisher
}

// NewKill_RequestPublisher creates and returns a new publisher for the
// Kill_Request
func NewKill_RequestPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*Kill_RequestPublisher, error) {
	pub, err := node.NewPublisher(topic_name, Kill_RequestTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &Kill_RequestPublisher{pub}, nil
}

func (p *Kill_RequestPublisher) Publish(msg *Kill_Request) error {
	return p.Publisher.Publish(msg)
}

// Kill_RequestSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type Kill_RequestSubscription struct {
	*rclgo.Subscription
}

// Kill_RequestSubscriptionCallback type is used to provide a subscription
// handler function for a Kill_RequestSubscription.
type Kill_RequestSubscriptionCallback func(msg *Kill_Request, info *rclgo.MessageInfo, err error)

// NewKill_RequestSubscription creates and returns a new subscription for the
// Kill_Request
func NewKill_RequestSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback Kill_RequestSubscriptionCallback) (*Kill_RequestSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg Kill_Request
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, Kill_RequestTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &Kill_RequestSubscription{sub}, nil
}

func (s *Kill_RequestSubscription) TakeMessage(out *Kill_Request) (*rclgo.MessageInfo, error) {
	return s.Subscription.TakeMessage(out)
}

// CloneKill_RequestSlice clones src to dst by calling Clone for each element in
// src. Panics if len(dst) < len(src).
func CloneKill_RequestSlice(dst, src []Kill_Request) {
	for i := range src {
		dst[i] = *src[i].Clone()
	}
}

// Modifying this variable is undefined behavior.
var Kill_RequestTypeSupport types.MessageTypeSupport = _Kill_RequestTypeSupport{}

type _Kill_RequestTypeSupport struct{}

func (t _Kill_RequestTypeSupport) New() types.Message {
	return NewKill_Request()
}

func (t _Kill_RequestTypeSupport) PrepareMemory() unsafe.Pointer { //returns *C.turtlesim__srv__Kill_Request
	return (unsafe.Pointer)(C.turtlesim__srv__Kill_Request__create())
}

func (t _Kill_RequestTypeSupport) ReleaseMemory(pointer_to_free unsafe.Pointer) {
	C.turtlesim__srv__Kill_Request__destroy((*C.turtlesim__srv__Kill_Request)(pointer_to_free))
}

func (t _Kill_RequestTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	m := msg.(*Kill_Request)
	mem := (*C.turtlesim__srv__Kill_Request)(dst)
	primitives.StringAsCStruct(unsafe.Pointer(&mem.name), m.Name)
}

func (t _Kill_RequestTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	m := msg.(*Kill_Request)
	mem := (*C.turtlesim__srv__Kill_Request)(ros2_message_buffer)
	primitives.StringAsGoStruct(&m.Name, unsafe.Pointer(&mem.name))
}

func (t _Kill_RequestTypeSupport) TypeSupport() unsafe.Pointer {
	return unsafe.Pointer(C.rosidl_typesupport_c__get_message_type_support_handle__turtlesim__srv__Kill_Request())
}

type CKill_Request = C.turtlesim__srv__Kill_Request
type CKill_Request__Sequence = C.turtlesim__srv__Kill_Request__Sequence

func Kill_Request__Sequence_to_Go(goSlice *[]Kill_Request, cSlice CKill_Request__Sequence) {
	if cSlice.size == 0 {
		return
	}
	*goSlice = make([]Kill_Request, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		Kill_RequestTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func Kill_Request__Sequence_to_C(cSlice *CKill_Request__Sequence, goSlice []Kill_Request) {
	if len(goSlice) == 0 {
		cSlice.data = nil
		cSlice.capacity = 0
		cSlice.size = 0
		return
	}
	cSlice.data = (*C.turtlesim__srv__Kill_Request)(C.malloc(C.sizeof_struct_turtlesim__srv__Kill_Request * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		Kill_RequestTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
func Kill_Request__Array_to_Go(goSlice []Kill_Request, cSlice []CKill_Request) {
	for i := 0; i < len(cSlice); i++ {
		Kill_RequestTypeSupport.AsGoStruct(&goSlice[i], unsafe.Pointer(&cSlice[i]))
	}
}
func Kill_Request__Array_to_C(cSlice []CKill_Request, goSlice []Kill_Request) {
	for i := 0; i < len(goSlice); i++ {
		Kill_RequestTypeSupport.AsCStruct(unsafe.Pointer(&cSlice[i]), &goSlice[i])
	}
}
