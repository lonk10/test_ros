...
import (
	...
	geo_msg "aburos/msgs/geometry_msgs/msg"
	mav_srv "aburos/msgs/mavros_msgs/srv"
...
type SubVehicle struct {
	...
	Subs           *rclgo.WaitSet
	keywords       []string
	funcs          map[string]func() // not sure if needed
}

func NewSubVehicle(lname string, gname string, extraKeywords []string) (*SubVehicle, error) {
	res := &SubVehicle{
		ID:             lname,
		Name:           "/mavros/" + lname,
		Domain:         gname,
	...
	res.keywords = append([]string{"start", "init", "position", "move", "mode"}, extraKeywords...)
	return res, nil
}
...
func (s *SubVehicle) Move(x float64, y float64, z float64) error {
